	...
	*/
	unbind: function (event, callback) {
		this.each(function () {
			...
			//if no events, cancel
			if (hdl && hdl[this[0]]){l = hdl[this[0]].length;} else {
                return this;
            }
			...
			if (filter) {
				...
				for (i = 0, l = results.length; i < l; i++) {
					...
					if (!found[id] && obj.x < rect._x + rect._w && obj._x + obj._w > rect._x &&
								 obj.y < rect._y + rect._h && obj._h + obj._y > rect._y){found[id] = results[i];}
				}
	...
	hit: function (comp) {
		...
		for (; i < l; ++i) {
			...
			if (!dupes[id] && this[0] !== id && obj.__c[comp] &&
							 ...
							 oarea._y < area._y + area._h && oarea._h + oarea._y > area._y){dupes[id] = obj;}
		}
...
Crafty.c("DOM", {
	...
	*/
	draw: function () {
		...
		if (!this._visible){style.visibility = "hidden";} else {style.visibility = "visible";}
...
Crafty.extend({
	...
	*/
	addEvent: function (ctx, obj, type, callback) {
		...
		if (!this._events[id + obj + type + callback]){this._events[id + obj + type + callback] = afn;} else {
            return;
        }
 ...
 Crafty.extend({
	...
	*/
	...
	touchDispatch: function(e) {
		...
		var type;
		if (e.type === "touchstart"){type = "mousedown";}else if (e.type === "touchmove"){type = "mousemove";}else if (e.type === "touchend"){type = "mouseup";}
	...
	animate: function (reelId, fromx, y, tox) {
		...
		if (arguments.length < 4 && typeof fromx === "number") {
			...
			if (arguments.length === 3 && typeof y === "number") {
				...
				if (y === -1){this._frame.repeatInfinitly = true;} else {this._frame.repeat = y;}
			}
        ...
        */
        mute:function(){
            ...
            if(!this.muted){
                ...
                for(var i in this.sounds){
                    s = this.sounds[i];
                    if(s.obj.currentTime && s.obj.currentTime > 0){this.sounds[i].obj.play();}
                }
                this.muted = false;
            }
